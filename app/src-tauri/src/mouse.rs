...
pub unsafe fn get_battery_status_with_handle(usb_handle: &mut driver::PlatformUsbDriver) -> Result<u8, String> {
...
pub unsafe fn get_polling_rate_with_handle(usb_handle: &mut driver::PlatformUsbDriver) -> Result<u16, String> {
...
pub unsafe fn get_backlight_with_handle(usb_handle: &mut driver::PlatformUsbDriver) -> Result<u8, String> {
...
pub unsafe fn get_dpi_xy_with_handle(usb_handle: &mut driver::PlatformUsbDriver) -> Result<(u16, u16), String> {
...
pub unsafe fn get_led_rgb_with_handle(usb_handle: &mut driver::PlatformUsbDriver) -> Result<[u8; 3], String> {
...
pub unsafe fn get_dpi_stages_with_handle(usb_handle: &mut driver::PlatformUsbDriver) -> Result<Vec<DpiStage>, String> {
...
pub unsafe fn set_dpi_stages_with_handle(usb_handle: &mut driver::PlatformUsbDriver, stages: Vec<DpiStage>) -> Result<(), String> {
    if stages.is_empty() {
        return Err("No DPI stages provided".to_string());
    }
    
    let active_dpi_stage = stages.iter().find(|s| s.active).map(|s| s.stage).unwrap_or(1);
